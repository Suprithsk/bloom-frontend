...
const PlatinumTab = ({ plans, loading, onSelectPlan }) => {
    if (loading) {
        return (
    ...
    if (!plans || plans.length === 0) {
        return (
            <div className="text-center py-8">
                <p className="text-muted-foreground">No Platinum plans available</p>
            </div>
        );
    }
            ...
            {plans.map((plan) => (
                <Card key={plan.plan_id} className="relative border-2 hover:border-green-500 transition-colors">
                    <CardHeader className="text-center">
                        <CardTitle className="text-lg">{plan.duration} PLATINUM</CardTitle>
